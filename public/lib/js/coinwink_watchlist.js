function watchlistAddCoin() {
  ...
  function addWatchCoin() {
    ...
    if (!found) { 
      updateWatchlistFeedback('added');
      watchlist.push({ "coin_id": coin_id, "note": "", "slug": website_slug }); 
    }
    else {
      updateWatchlistFeedback('alreadyInList');
    }
  }
...
function watchlistRemoveCoin(coin_id) {
  console.log('1', coin_id)
  if (typeof(coin_id) == 'undefined') {
    var coin_id = jQuery("#watchlist_dropdown").val();
  }

  console.log('2', coin_id)
  for (var i = 0; i < watchlist.length; i++) {
    if (watchlist[i]["coin_id"] === coin_id) {
      watchlist.splice(i, 1);
      updateWatchlistFeedback('removed');
      break;
    }
    else {
      updateWatchlistFeedback('notInList');
    }
  }
    ...
    function build_table(json) {
      ...
      watchlistNew.map(function (row) {
        ...
        bodyRows += '<div class="watchlist-grid"><div width="60" class="watchlistcoinid" id="' +
          ...
          row["slug"] +
          '/"><div style="position:absolute;left:9px;margin-top:-2px;"><img src="/img/coins/32x32/' +
          row["coin_id"] +
          '.png" width="18"></div><div style="padding-left:31px;width:40px;text-align:left;">' +
...
function updateWatchlistFeedback(type) {
  var divId = Date.now();
  if(type == 'added') {
    document.getElementById('watchlist-feedback').innerHTML = '<div id="'+divId+'">Added!</div>';
  }
  else if (type == 'removed') {
    document.getElementById('watchlist-feedback').innerHTML = '<div id="'+divId+'">Removed!</div>';
  }
  else if (type == 'notInList') { 
    document.getElementById('watchlist-feedback').innerHTML = '<div id="'+divId+'">Not in watchlist!</div>';
  }
  else if (type == 'alreadyInList') {
    document.getElementById('watchlist-feedback').innerHTML = '<div id="'+divId+'">Already in watchlist!</div>';
  }
  setTimeout(function () {
